...
CPFA_controller::CPFA_controller() : RNG(argos::CRandom::CreateRNG("argos")),
																		 isInformed(false),
																		 isHoldingFood(false),
...
void CPFA_controller::ControlStep()
{
	...
	*/
	...
	// =========================================================================
	// 🌟 [新增] 真正的 ARGoS 去中心化通信逻辑 (2米内检测)
	// =========================================================================
	// 只有当机器人在返巢（带着物资回去）或者已经被告知了信息素航点时，才有信息可分享
	if (CPFA_state == RETURNING || isInformed)
	{

		// 1. 获取 ARGoS 仿真的核心“空间”对象
		argos::CSpace &cSpace = argos::CSimulator::GetInstance().GetSpace();

		// 2. 从空间中获取所有的 foot-bot（即我们的机器人实体）
		argos::CSpace::TMapPerType &m_cFootbots = cSpace.GetEntitiesByType("foot-bot");
				 ...
				 ++it)
		{
			...
			// 获取当前遍历到的机器人实体（转换为 CFootBotEntity）
			argos::CFootBotEntity &cFootBot = *argos::any_cast<argos::CFootBotEntity *>(it->second);

			// 获取它的唯一 ID 和当前位置
			std::string other_id = cFootBot.GetId();
			argos::CVector3 other_pos3d = cFootBot.GetEmbodiedEntity().GetOriginAnchor().Position;

			// 将 3D 坐标降维成 2D 坐标（因为我们只关心平面距离）
			argos::CVector2 other_pos(other_pos3d.GetX(), other_pos3d.GetY());
			...
			// 计算距离
			argos::Real distance = (this->GetPosition() - other_pos).Length();

			// 检查距离是否在 2 米以内
			if (distance <= 2.0)
			{
				argos::CControllableEntity &cControllable = cFootBot.GetControllableEntity();
				CPFA_controller &cOtherController = dynamic_cast<CPFA_controller &>(cControllable.GetController());

				// 1. 提取官方列表里的信息素
				std::vector<Pheromone> myPheromones = this->GetLocalPheromoneList();

				// 2. 🌟 核心修复：如果自己脑子里正记着一个刚发现的物资点，直接打包塞进去！
				if (isUsingSiteFidelity)
				{
					argos::Real timeInSec = (argos::Real)(SimulationTick() / SimulationTicksPerSecond());
					Pheromone livePheromone(SiteFidelityPosition, TrailToShare, timeInSec, LoopFunctions->RateOfPheromoneDecay, ResourceDensity);
					myPheromones.push_back(livePheromone);
				}

				// 3. 把打包好的最新情报发送给对方
				cOtherController.MergeSharedPheromones(myPheromones);

				// 4. 🌟 硬盘保护机制：只有包裹里真有情报时，才打印日志！杜绝空包刷屏！
				if (myPheromones.size() > 0)
				{
					argos::LOG << "[DEBUG] Time: " << SimulationTick()
										 << " | Robot " << GetId() << " transmitted data to Robot " << other_id
										 << " (Dist: " << distance << "m). Payload (" << myPheromones.size() << " wp): [";

					for (size_t p = 0; p < myPheromones.size(); p++)
					{
						argos::LOG << "{" << myPheromones[p].GetLocation().GetX() << ", "
											 << myPheromones[p].GetLocation().GetY() << " | Wt: "
											 << myPheromones[p].GetWeight() << "}";
						if (p < myPheromones.size() - 1)
							argos::LOG << ", ";
					}
					argos::LOG << "]" << std::endl;
				}
...
bool CPFA_controller::SetTargetPheromone()
{
	...
	if (LocalPheromoneList.size() == 0)
		return isPheromoneSet; // the case of no pheromone.
...
size_t CPFA_controller::GetSearchingTime()
{ // qilu 10/22
	return searchingTime;
}
size_t CPFA_controller::GetTravelingTime()
{ // qilu 10/22
	return travelingTime;
}

string CPFA_controller::GetStatus()
{ // qilu 10/22
	// DEPARTING, SEARCHING, RETURNING
	if (CPFA_state == DEPARTING)
		return "DEPARTING";
	else if (CPFA_state == SEARCHING)
		return "SEARCHING";
	else if (CPFA_state == RETURNING)
		return "RETURNING";
	else if (CPFA_state == SURVEYING)
		return "SURVEYING";
